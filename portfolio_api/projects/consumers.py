    ...
    async def receive(self, text_data):
        try:
            ...
            if 'command' in data:
                ...
                # If the command is a terminal control character, just pass it through
                if len(command) == 1 and ord(command) < 32:
                    output = await self.pty.run(command, timeout=1)
                    await self.send(text_data=json.dumps({
                        'output': output
                    }))
                    return
                
                # For normal commands, validate and execute
                ...
                output = await self.pty.run(command, timeout=10)
                await self.send(text_data=json.dumps({
                    'output': output
                }))
